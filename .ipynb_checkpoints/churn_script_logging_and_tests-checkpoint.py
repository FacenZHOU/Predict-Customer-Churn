...
def test_perform_feature_engineering(data_frame):
    ...
    '''
    X_train, X_test, y_train, y_test = cl.perform_feature_engineering(data_frame)
    try:
        assert len(X_train) == len(y_train)
        assert len(X_test) == len(y_test)
        logging.info('SUCCESS: Datasets for training and testing have been correctly generated')
    except AssertionError as err:
